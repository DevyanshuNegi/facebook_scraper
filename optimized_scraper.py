...
class OptimizedFacebookScraper:
    ...
    def __init__(self, max_concurrent_pages=10, browser_timeout=60000):
        self.max_concurrent_pages = max_concurrent_pages
        self.browser_timeout = browser_timeout
        self.browser: Optional[Browser] = None
        self.context: Optional[BrowserContext] = None
        self.cookies_loaded = False
        self.page_pool = asyncio.Queue(maxsize=max_concurrent_pages)
        ...
        # CSS selectors for targeted email search (faster than full page text)
    ...
    async def initialize(self, cookie_file_path: str):
            ...
            try:
                ...
                for url in urls_to_try:
                    try:
                        ...
                        for selector in self.email_selectors[:3]:  # Only check first 3 selectors for speed
                            try:
...
async def get_scraper_instance(cookie_file_path: str = 'fb_cookies.json') -> OptimizedFacebookScraper:
    ...
    if _scraper_instance is None:
        with _scraper_lock:
            if _scraper_instance is None:
                _scraper_instance = OptimizedFacebookScraper(max_concurrent_pages=15)
                await _scraper_instance.initialize(cookie_file_path)
    
    return _scraper_instance

# Backwards compatibility function
async def get_email_from_page_optimized(fb_url: str, cookie_file_path: str = 'fb_cookies.json') -> Optional[str]:
...
def get_email_from_page_sync(fb_url: str, cookie_file_path: str = 'fb_cookies.json') -> Optional[str]:
